...
class Model_E2pdf_Revision extends Model_E2pdf_Model {
    ...
    public function load($template_id, $full = true, $revision_id = 0) {
        ...
        if ($template && $revision_id) {
            ...
            if ($full) {
                $this->set('fonts', unserialize($template['fonts'], array('allowed_classes' => false)));
                $this->set('actions', unserialize($template['actions'], array('allowed_classes' => false)));
    ...
    public function revision($template_id = false) {
        ...
        if ($template_id) {
            ...
            if ($template) {
                ...
                $this->set('fonts', unserialize($template['fonts'], array('allowed_classes' => false)));
                $this->set('actions', unserialize($template['actions'], array('allowed_classes' => false)));
                $this->set('permissions', unserialize($template['permissions'], array('allowed_classes' => false)));
    ...
    public function get($key) {
        if (isset($this->template[$key])) {
            if ($key == 'permissions') {
                $value = unserialize($this->template[$key], array('allowed_classes' => false));
            } else {
