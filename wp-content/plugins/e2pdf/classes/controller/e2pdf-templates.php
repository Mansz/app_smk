    ...
    public function import_action() {
        ...
        if ($this->post->get('_nonce')) {
            ...
            if (!$tmp) {
                ...
                $pages = @unserialize(base64_decode((string) $xml->template->pages), array('allowed_classes' => false));
                if (is_array($pages)) {
                ...
                if (isset($xml->template->permissions)) {
                    $template->set('permissions', @unserialize(base64_decode((string) $xml->template->permissions), array('allowed_classes' => false)));
                } else {
                ...
                $template->set('fonts', @unserialize(base64_decode((string) $xml->template->fonts), array('allowed_classes' => false)));
                $template->set(
                        'actions',
                        apply_filters('e2pdf_controller_templates_import_actions', @unserialize(base64_decode((string) $xml->template->actions), array('allowed_classes' => false)), $options, $xml, $template, $extension)
                );
                $template->set(
                        'pages',
