...
class Extension_E2pdf_Woocommerce extends Model_E2pdf_Model {
    ...
    public function load_actions() {
        ...
        if (get_option('e2pdf_wc_order_details_template_id') && get_option('e2pdf_wc_order_details_template_id_hook') &&
                in_array(
                        get_option('e2pdf_wc_order_details_template_id_hook'),
                        array(
            ...
            add_action(get_option('e2pdf_wc_order_details_template_id_hook'), array($this, 'action_e2pdf_wc_order_details_template_id'), $priority);
        }
                ...
                if ($modifier && $modifier != 'text') {
                    ...
                    if ($list_id !== false) {
                        ...
                        if (is_serialized($raw_value)) {
                            $list = @unserialize(trim($raw_value), array('allowed_classes' => false));
                        } else {
                            $list = $raw_value;
                        }
                        if (is_array($list)) {
    ...
    public function render_shortcodes($value, $field = array()) {
                ...
                if (!empty($tagnames)) {
                    ...
                    foreach ($shortcodes[0] as $key => $shortcode_value) {
                        ...
                        if ($shortcode[2] === 'e2pdf-user') {
                            ...
                            } elseif (isset($atts['id']) && $atts['id'] == 'dynamic' && substr($shortcode_value, -13) === '[/e2pdf-user]') {
                                if ($shortcode[5]) {
        ...
        return apply_filters(
                'e2pdf_extension_render_shortcodes_value', $value, $element_id, $template_id, $item, $dataset, false, false
        );
    }
    ...
    public function verify() {
        if ($this->get('item') && $this->get('dataset') && get_post($this->get('dataset')) && ($this->get('item') == get_post_type($this->get('dataset')) || ($this->get('item') == 'cart' && $this->get('dataset') == wc_get_page_id('cart')))) {
            return true;
        }
        return false;
    }
    ...
    private function get_product_attribute_keys() {
        ...
        foreach ($custom_meta_keys as $custom_meta_key) {
            $custom_metas = @unserialize($custom_meta_key, array('allowed_classes' => false));
