...
class Extension_E2pdf_Forminator extends Model_E2pdf_Model {
    ...
    public function render_shortcodes($value, $field = array()) {
        ...
        if ($this->verify() && function_exists("forminator_replace_form_data") && class_exists("Forminator_API")) {
            ...
            $args = apply_filters('e2pdf_extension_render_shortcodes_args', $args, $element_id, $template_id, $this->get('item'), $this->get('dataset'), false, false);
            ...
            if ($form->is_prevent_store()) {
                if (class_exists('Forminator_Form_Entry_Model')) {
                    ...
                    if ($this->get('field_data_array') && is_array($this->get('field_data_array'))) {
                        $entry->set_fields($this->replace_values_to_labels($this->get('field_data_array'), $form, $entry));
            ...
            if (false !== strpos($value, '[')) {
                $replace = array(
                    '[e2pdf-dataset]' => $this->get('dataset'),
                    '[pdf_url]' => '[e2pdf-url]',
                    '[e2pdf-url]' => '',
                );
                ...
                if (!empty($tagnames)) {
                    ...
                    foreach ($shortcodes[0] as $key => $shortcode_value) {
                        ...
                        if ($shortcode[2] === 'e2pdf-user') {
                            if ((!isset($atts['id']) && $this->get('user_id')) || (isset($atts['id']) && $atts['id'] == 'dynamic')) {
                                if (!isset($atts['id'])) {
                                    $shortcode[3] .= ' id="' . $this->get('user_id') . '"';
                                }
                            ...
                            if (isset($atts['key']) && isset($args[$atts['key']])) {
                                $value = str_replace($shortcode_value, $this->strip_shortcodes($args[$atts['key']]), $value);
                            } else {
            ...
            $value = apply_filters('e2pdf_extension_render_shortcodes_pre_do_shortcode', $value, $element_id, $template_id, $this->get('item'), $this->get('dataset'), false, false);
            $value = do_shortcode($value);
            $value = apply_filters('e2pdf_extension_render_shortcodes_after_do_shortcode', $value, $element_id, $template_id, $this->get('item'), $this->get('dataset'), false, false);
            ...
            if (defined('FORMINATOR_VERSION') && version_compare(FORMINATOR_VERSION, '1.16.0', '>=')) {
                ...
                if (version_compare(FORMINATOR_VERSION, '1.17.2', '>=')) {
                    $value = forminator_replace_variables($value, $this->get('item'), $entry);
                } else {
            ...
            $value = apply_filters('e2pdf_extension_render_shortcodes_pre_value', $value, $element_id, $template_id, $this->get('item'), $this->get('dataset'), false, false);
        ...
        return apply_filters(
                'e2pdf_extension_render_shortcodes_value', $value, $element_id, $template_id, $this->get('item'), $this->get('dataset'), false, false
        );
    }
    ...
    public function verify() {
        if ($this->get('item') && $this->get('dataset') && class_exists('Forminator_API')) {
            $form = Forminator_API::get_form($this->get('item'));
            if (!is_wp_error($form)) {
                if ($form->is_prevent_store()) {
                    ...
                    $entry = Forminator_API::get_entry($this->get('item'), $this->get('dataset'));
                    if (!is_wp_error($entry) && $entry->form_id == $this->get('item')) {
                        return true;
                    }
                }
            }
