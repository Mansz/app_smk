    ...
    public function render_shortcodes($value, $field = array()) {
        ...
        if ($this->verify()) {
            ...
            if (false !== strpos($value, '[')) {
                ...
                /* Render Separatable fields shortcode */
                ...
                foreach ($matches[1] as $match) {
                    if ($this->get('item') == '-2') {
                        if ($this->get('dataset') && $this->get('dataset2')) {
                            ...
                            $new_match = apply_filters('frm_content', $new_match, $form2, $entry2);
                        } elseif ($this->get('dataset')) {
                            $new_match = apply_filters('frm_content', $match, $form, $entry);
                            if ($form2) {
                                $new_match = apply_filters('frm_content', $new_match, $form2, $entry);
                            }
                        } elseif ($this->get('dataset2')) {
                            $new_match = apply_filters('frm_content', $match, $form2, $entry2);
                            if ($form) {
                                $new_match = apply_filters('frm_content', $new_match, $form, $entry2);
                            }
                        }
                    } else {
                        $new_match = apply_filters('frm_content', $match, $form, $entry);
            ...
            if ($this->get('item') == '-2') {
                if ($this->get('dataset') && $this->get('dataset2')) {
                    ...
                    $value = apply_filters('frm_content', $value, $form, $entry);
                    if ($form2) {
                        $value = apply_filters('frm_content', $value, $form2, $entry);
                    }
                } elseif ($this->get('dataset2')) {
                    $value = apply_filters('frm_content', $value, $form2, $entry2);
                    if ($form) {
                        $value = apply_filters('frm_content', $value, $form, $entry2);
                    }
                }
            } else {
                $value = apply_filters('frm_content', $value, $form, $entry);
                ...
                if (false !== strpos($value, '[')) {
                    ...
                    if ((false !== strpos($value, '[referer]') || false !== strpos($value, '[browser]')) || (false !== strpos($value, '[referer2]') || false !== strpos($value, '[browser2]'))) {
                        ...
                        if ($entry && isset($entry->description)) {
                            if (is_serialized($entry->description)) {
                                $description = @unserialize(trim($entry->description), array('allowed_classes' => false));
                            } else {
                        ...
                        if ($entry2 && isset($entry2->description)) {
                            if (is_serialized($entry2->description)) {
                                $description = @unserialize(trim($entry2->description), array('allowed_classes' => false));
                            } else {
        ...
        return apply_filters(
                'e2pdf_extension_render_shortcodes_value', $value, $element_id, $template_id, $this->get('item'), $this->get('dataset'), $this->get('item2'), $this->get('dataset2')
        );
    }
