...
class Extension_E2pdf_Caldera extends Model_E2pdf_Model {
    ...
    public function item($item = false) {
        ...
        if ($caldera_form) {
            ...
            $form->url = $this->helper->get_url(
                    array(
                        'page' => 'caldera-forms',
                        'edit' => $caldera_form['ID'],
                    )
            );
            $form->name = isset($caldera_form['name']) ? $caldera_form['name'] : '';
        } else {
            $form->id = '';
    ...
    public function verify() {
        if ($this->get('item') && $this->get('dataset') && class_exists('Caldera_Forms_Entry') && class_exists('Caldera_Forms_Forms') && class_exists("Caldera_Forms")) {
            $form = Caldera_Forms_Forms::get_form($this->get('item'));
            if ($form && isset($form['_last_updated'])) {
                $entry = new Caldera_Forms_Entry($form, $this->get('dataset'));
                if ($entry->found()) {
                    return true;
                }
            }
                ...
                if ($last && is_array($last) && isset($last['value'])) {
                    ...
                    if (!empty($is_json)) {
                        ...
                        if (is_serialized($last['value'])) {
                            $serialized = @unserialize($last['value'], array('allowed_classes' => false));
                            if (!empty($serialized)) {
                                $entry = array(
                                    '0' => array(
                                        'value' => $serialized,
